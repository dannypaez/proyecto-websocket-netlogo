...
to setup
  ...
  set max-min-speed-table table:make
...
to go
  ...
  if ticks mod 2 = 0 [
    let speed-max-car max [speed] of turtles
    let speed-min-car min [speed] of turtles
    let red-car-speed [speed] of sample-car

    table:put max-min-speed-table ticks (list speed-max-car speed-min-car red-car-speed)
...
to connection
  let data (table:to-list max-min-speed-table)
  let data_names (list "Max Speed" "Min Speed" "Red Car Speed")

  ; Usar el nuevo comando simplificado para enviar datos
  ifelse websocket-extension:is-open [
