extensions [websocket-extension table]

globals [
  grass-and-weeds-table
]
...
to setup
  ...
  create-rabbits number [
    ...
    set energy random 10  ; Empiezan con una cantidad aleatoria de energía
  ]

  set grass-and-weeds-table table:make

  ; Conectar al servidor WebSocket
  websocket-extension:connect-websocket "ws://127.0.0.1:5678"
  ifelse websocket-extension:is-open [
    show "Conectado al servidor WebSocket."
  ] [
    show "No se pudo conectar al servidor WebSocket."
  ]
...
to go
  ...
  if ticks mod 1 = 0 [
    let total-rabbits count rabbits
    let percent-grass (count patches with [pcolor = green]) / 4
    let percent-weeds (count patches with [pcolor = violet]) / 4
    table:put grass-and-weeds-table ticks (list total-rabbits percent-grass percent-weeds)
...
to grow-grass-and-weeds
  ask patches [
    if pcolor = black [
      if random-float 1000 < weeds-grow-rate [ set pcolor violet ]
      if random-float 1000 < grass-grow-rate [ set pcolor green ]
    ]
  ]
end
...
to eat-grass  ;; rabbit procedure
  ;; Ganar "grass-energy" comiendo pasto
  if pcolor = green [
    set pcolor black
...
to eat-weeds  ;; rabbit procedure
  ;; Ganar "weed-energy" comiendo maleza
  if pcolor = violet [
    set pcolor black
...
to reproduce     ;; rabbit procedure
  ;; Dar a luz a un nuevo conejo, pero cuesta mucha energía
...
to death     ;; rabbit procedure
  ;; Morir si te quedas sin energía
  if energy < 0 [ die ]
end

to connection
  let data (table:to-list grass-and-weeds-table)
  let data_names (list "Rabbits" "Grass" "Weeds")

  ; Usar el comando 'send-data' de la extensión WebSocket para enviar los datos
  ifelse websocket-extension:is-open [
    websocket-extension:send-data data data_names
    show "Datos enviados al servidor WebSocket."
  ] [
    show "La conexión WebSocket no está abierta."
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
